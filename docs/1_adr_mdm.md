...
Data: 2026-06-23. Última atualização: 2026-07-07 (+ADR-008, +ADR-009, +ADR-010, +ADR-011, +ADR-012, +ADR-013, +ADR-014).
...
## ADR-014 — Proveniência do afastamento multi-fonte: §4.21 é a fonte do evento; §4.1 é face-FOTO; Ocorrências e PSS não ingerem

**Decisão.** O mesmo afastamento aparece em quatro faces SIAPE; a autoridade se resolve **por natureza do dado (ADR-001), não por qualidade da fonte**. A regra: (1) o **evento** `AFASTAMENTO` (`intercorrencias`) é alimentado **exclusivamente** por `consultaDadosAfastamentoHistorico` (WS_SIAPE_CONSULTAS **§4.21**) — série datada completa, `cod_mecanica=ingestao`, `grau_confianca=alto`, `fonte='WS_SIAPE:consultaDadosAfastamentoHistorico'`; (2) a **FOTO** (`cod_afastamento_vigente`) é alimentada por `consultaDadosAfastamento` (**§4.1**, vigente) — face-FOTO do mesmo fato, resolvida no D-1, **nunca vira evento**; (3) `listaContribuicoesPSS` (**§4.22**) permanece **atributo** do payload `CONTRIBUICAO_PSS` (insumo de dias-líquidos), **nunca evento** — já fechado no catálogo v1.2, aqui só reafirmado; (4) a **API SIAPE Ocorrências** (write) fica **fora da ingestão** — nem FOTO nem evento leem dela. Não há empate a desempatar em runtime: **cada face tem uma única fonte por construção**, e proveniência (`fonte`, `cod_mecanica`, `grau_confianca`) viaja no envelope do evento (ADR-006) apenas para rastro, não para arbitragem entre fontes concorrentes do mesmo registro.

**Contexto.** É o **primeiro evento do projeto que nasce de N fontes** (prespec §7.1), e a pendência foi enunciada como "qual fonte prevalece quando o mesmo afastamento aparece em mais de uma". Ao ler as fichas (`3_catalogo_fontes_v0_1.md` §1/§2), a pergunta se dissolve: as quatro faces não competem pelo mesmo slot. **§4.1** entrega só afastamentos `≥ data da consulta` — é estado vigente, não série; casá-lo com evento seria historizar a foto, o anti-padrão que a ADR-001 proíbe. **§4.21** entrega a série datada encerrada — é a única que satisfaz a completude exaustiva que o evento exige (cálculo de efetivo exercício, KR 2.2; réplica de trajetória no Filme). **§4.22** carrega arrays `ferias/lpa/afastamentos/reclusao` como sub-estrutura de `CONTRIBUICAO_PSS` para dias-líquidos — mesmo fato, mas o payload PSS não é o registro do afastamento, é o da contribuição; duplicar viraria evento-espelho (recusado por padrão, ADR-008). **API Ocorrências** também tem histórico (`situacaoAfastamento=5`), mas é **redundante e mais cara** que §4.21: trava no órgão do gestor (nível ÓRGÃO), exige que o CPF do certificado ICP-Brasil assine cada requisição, e é o canal de *escrita* de frequência (Falta/Licença) — o MDM é SoR read-only e não consome canal de escrita quando uma consulta pura entrega a mesma série. A confusão latente era tratar "quatro fontes do mesmo fato" como "quatro candidatas ao mesmo campo"; são faces de naturezas distintas (FOTO × evento × atributo-de-outro-evento × sistema-externo).

**Opções consideradas.**
1. **Desempate por `grau_confianca` entre fontes concorrentes** (ex.: §4.21 vs. Ocorrências para o mesmo afastamento, vale o de maior confiança). Rejeitada — pressupõe que duas fontes gravam o mesmo evento e o pipeline arbitra; mas só uma fonte alimenta o evento, então não há concorrência a arbitrar. Introduziria máquina de desempate para um empate que o desenho não produz (peça que não paga aluguel).
2. **Ocorrências como fonte primária** (é a mais "rica": tem `gerarEfeitoFinanceiro`, `anoMesCadastro`). Rejeitada — riqueza marginal não justifica o custo de acesso (assinatura ICP-Brasil por chamada, trava de órgão, canal de escrita) quando §4.21 é consulta pura com a mesma série datada; e o corte financeiro do afastamento já vem por `dom_afastamento.conta_efetivo_exercicio` + o próprio FECHAMENTO_FOLHA, não precisa do flag de Ocorrências.
3. **§4.1 + §4.21 ambos viram evento** (vigente + histórico, dedup na projeção). Rejeitada — §4.1 é estado, não série; alimentá-lo ao evento historiza a foto e duplica o vigente (que §4.21 já cobre no seu extremo recente). §4.1 fica onde pertence: FOTO.
4. **§4.21 fonte única do evento; §4.1 face-FOTO; §4.22 atributo; Ocorrências fora.** Escolhida — cada face na sua natureza; zero peça de arbitragem; proveniência é rastro, não árbitro.

**Resultado.** Regra de fronteira escrita, **sem mudança de schema** (o envelope já carrega `fonte`/`cod_mecanica`/`grau_confianca`; o catálogo já fixou §4.21 no tipo AFASTAMENTO e §4.22 como atributo — v1.2). Carimbo canônico do evento AFASTAMENTO na ingestão: `fonte='WS_SIAPE:consultaDadosAfastamentoHistorico'`, `cod_mecanica='ingestao'`, `grau_confianca='alto'`. **Fonte confirmada no WSDL (`ConsultaSIAPE?wsdl`, 2026-07-07):** `consultaDadosAfastamentoHistoricoResponse` devolve `ArrayOfArrayDadosAfastamento` (série por vínculo × afastamentos), distinto de `ArrayOfArrayDadosFinanceiros` de §4.20 — a tabela de rubricas impressa na página de §4.21 do `WS_SIAPE_CONSULTAS.pdf` era deslize de diagramação (saída de §4.20 deslocada uma seção), não o shape real da operação. Ocorrências é fallback **descartado por custo**, não fallback-necessário. Nota residual: o shape fino de campos de `DadosAfastamento` não está no WSDL (`<xsd:types>` vazio, complexType em schema importado); o payload do evento não depende dele (campos já ancorados em §4.1 e no array de §4.22, mesmo eixo `dom_afastamento`), e o de-para fino fecha quando o acesso à API SIAPE Consultas abrir e um retorno real for capturado. **Princípio geral que esta ADR estabelece:** quando um fato aparece em N fontes SIAPE, a autoridade se decide pela **natureza que cada face serve** (FOTO / evento / atributo de outro evento), não por ranking de fonte — só há disputa de proveniência real quando *duas fontes alimentam a mesma face*, o que aqui não ocorre. Se um dia o retroativo pré-SIAPE (papel/OCR, `grau_confianca='baixo'`) trouxer afastamentos para o **mesmo** intervalo já coberto por §4.21, **aí sim** nasce concorrência de fonte para a mesma face-evento — e o desempate será `grau_confianca` + `data_carga` (coalescência ADR-008), a registrar como extensão desta ADR quando o Extrator materializar o caso. Hoje é inerte (fonte única). Fronteira de sub-domínio (`intercorrencias`) não reaberta — dona é o RH (Seção 2). Decisão da mesa de engenharia (Tech Lead), 2026-07-07.
